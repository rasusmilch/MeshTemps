 ...
 * @link
 *
 * This file contains all the configurations needed for a custom board using ESP
 * Panel. Users can modify these configurations according to their hardware
 * design.
 */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////// Please update the following macros to configure
///general parameters ///////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////// Please update the following macros to configure the
///LCD panel /////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 ...
 *
 * Configure parameters based on the selected bus type. Parameters for other bus
 * types will be ignored. For detailed parameter explanations, see:
 ...
 */
/**
 * Set to 0 if using simple "RGB" interface which does not contain "3-wire SPI"
 * interface.
 */
...
#define ESP_PANEL_BOARD_LCD_RGB_PCLK_ACTIVE_NEG                                \
  (1) // 0: rising edge, 1: falling edge. Typically set to 0
...
#define ESP_PANEL_BOARD_LCD_RGB_PIXEL_BITS                                     \
  (ESP_PANEL_LCD_COLOR_BITS_RGB565) // | ESP_PANEL_LCD_COLOR_BITS_RGB565 |
...
#define ESP_PANEL_BOARD_LCD_RGB_IO_DISP                                        \
  (-1) // -1 if not used. Typically set to -1
...
// ┗------|--------------|--------|--------|--------|
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA0                                       \
  (14) //        |      D0      |   B0   |  B0-1  |   B0-3 |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA1                                       \
  (38) //        |      D1      |   B1   |  B2    |   B4   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA2                                       \
  (18) //        |      D2      |   B2   |  B3    |   B5   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA3                                       \
  (17) //        |      D3      |   B3   |  B4    |   B6   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA4                                       \
  (10) //        |      D4      |   B4   |  B5    |   B7   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA5                                       \
  (39) //        |      D5      |   G0   |  G0    |   G0-2 |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA6                                       \
  (0) //        |      D6      |   G1   |  G1    |   G3   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA7                                       \
  (45) //        |      D7      |   G2   |  G2    |   G4   |
#if ESP_PANEL_BOARD_LCD_RGB_DATA_WIDTH >                                       \
    8 //        ┗--------------┫--------|--------|--------|
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA8                                       \
  (48) //                       |   G3   |  G3    |   G5   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA9                                       \
  (47) //                       |   G4   |  G4    |   G6   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA10                                      \
  (21) //                       |   G5   |  G5    |   G7   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA11                                      \
  (1) //                       |   R0   |  R0-1  |   R0-3 |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA12                                      \
  (2) //                       |   R1   |  R2    |   R4   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA13                                      \
  (42) //                       |   R2   |  R3    |   R5   |
#define ESP_PANEL_BOARD_LCD_RGB_IO_DATA14                                      \
  (41) //                       |   R3   |  R4    |   R6   |
...
/**
 * If set to 1, the bus will skip to initialize the corresponding host. Users
 * need to initialize the host in advance.
 *
 * For drivers which created by this library, even if they use the same host,
 * the host will be initialized only once. So it is not necessary to set the
 * macro to `1`. For other drivers (like `Wire`), please set the macro to `1`
...
#define ESP_PANEL_BOARD_TOUCH_RST_LEVEL                                        \
  (0)                                    // Reset active level, 0: low, 1: high
#define ESP_PANEL_BOARD_TOUCH_INT_IO (4) // Interrupt pin, -1 if not used
#define ESP_PANEL_BOARD_TOUCH_INT_LEVEL                                        \
  (0) // Interrupt active level, 0: low, 1: high
 ...
 *
 * Supported types:
 * - `ESP_PANEL_BACKLIGHT_TYPE_SWITCH_GPIO`: Use GPIO switch to control the
 * backlight, only support on/off
 * - `ESP_PANEL_BACKLIGHT_TYPE_SWITCH_EXPANDER`: Use IO expander to control the
 * backlight, only support on/off
 * - `ESP_PANEL_BACKLIGHT_TYPE_PWM_LEDC`: Use LEDC PWM to control the backlight,
 * support brightness adjustment
 * - `ESP_PANEL_BACKLIGHT_TYPE_CUSTOM`: Use custom function to control the
 * backlight
 */
#define ESP_PANEL_BOARD_BACKLIGHT_TYPE                                         \
  (ESP_PANEL_BACKLIGHT_TYPE_SWITCH_EXPANDER)

#if (ESP_PANEL_BOARD_BACKLIGHT_TYPE ==                                         \
     ESP_PANEL_BACKLIGHT_TYPE_SWITCH_GPIO) ||                                  \
    (ESP_PANEL_BOARD_BACKLIGHT_TYPE ==                                         \
     ESP_PANEL_BACKLIGHT_TYPE_SWITCH_EXPANDER) ||                              \
 ...
 *
 * Set to 1 if want to turn off the backlight after initializing. Otherwise, the
 ...
 * For drivers which created by this library, even if they use the same host,
 * the host will be initialized only once. So it is not necessary to set the
 * macro to `1`. For other devices, please set the macro to `1` ensure that the
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////// Please utilize the following macros to execute any
